...
const Suppliers = () => {
    ...
    const fetchSuppliers = async () => {
        try {
            const response = await fetch('http://192.168.1.36:3000/api/suppliers');
            if (!response.ok) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const url = isEditing
                ? `http://192.168.1.51:3000/api/update-supplier/${selectedSupplier._id}`
                : 'http://192.168.1.51:3000/api/add-supplier';
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Bạn có chắc chắn muốn xóa nhà cung cấp này không?')) {
            try {
                const response = await fetch(`http://192.168.1.36:3000/api/delete-supplier-by-id/${id}`, {
                    method: 'DELETE',
                });
                const result = await response.json();
